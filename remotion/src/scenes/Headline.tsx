 ...
 *
 * **実測でこの近似は文字数/行の伸びを過大評価していた。** フレーズ境界
 * （ZWSP）でしか折り返さないため、フォントを縮めても「1行に入るフレーズの
 * 数」は理論値（フォントサイズに反比例）ほど増えない。行の末尾には
 * 「次のフレーズが入り切らない」端数が必ず残り、その端数はフォントを
 * 縮めても消えない。
 *
 * **係数が要るかどうかは同じ字数でも言い回しで変わる**（フレーズの長さが
 * 変わるため）。`compare` の 380px ゾーンに45字を入れた実測:
 *
 * - `推論コストが一桁下がる理由を…解き明かします`
 *   → 係数なし（≈69.7px）でも4行で収まった
 * - `大規模言語モデルの推論基盤における計算資源配分最適化技術が…発表です`
 *   → 係数なしでは5行必要になり、5行目 `発表です` が**上下に切れた**。
 *     係数 0.85（≈59px）で4行に収まり全文が出た
 *
 * つまり係数なしでは「収まる言い回しと収まらない言い回しがある」状態で、
 * 収まらない側に落ちたときの実害（文字が切れる）の方が、常に少し小さく
 * 描く実害より大きい。長い複合語が続く見出しほど端数が大きくなる。
 */
