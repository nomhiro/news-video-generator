...
const RATIOS = {
  ...
  shared: {
    ...
    // 章タグはこの帯（挿絵の下端寄り120px）の中に左寄せで置く。
    // 全幅ではなく挿絵の中に重ねるので、独立したゾーンとして持つ。
    //
    // **挿絵の下端ぎりぎりに寄せている。** 実写（テスト画像ではなく実際に
    // 生成した挿絵）で確認したところ、章タグが円形アイコンの1つに重なって
    // 隠していた。挿絵に重ねる以上、何かを覆う可能性はゼロにできない
    // （「文字を置く場所」と「絵の被写体」は独立に決まる）ので、
    // 少しでも被害を減らす——`Illustration.tsx` の下端フェード
    // （78%〜100%で透明度が落ちる）が最も強く効く範囲まで沈める。
    // 高さを180→120に削って中心をさらに下げた分、フェードが深い位置に
    // 収まる（旧: 中心792px・フェード進捗37%→新: 中心822px・52%）。
    chapter: { top: 800 / 1920, height: 120 / 1920 },
  },
