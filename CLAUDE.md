...
- **帯の中身は軌道（`h-16` = 64px）に収める。軌道を伸ばして直さない。**
  点・時刻・状態の語で 24（`top-6`）+ 10 + 2 + 15 + 2 + 15 = **68px** あり、
  `overflow-hidden` が**状態の語の行を4px断ち切っていた**（2026-08-23 に実測。
  `a239dc3` は 2026-08-22 17:06、指摘は翌朝 09:54 なので、読めない帯が動いて
  いたのは約17時間）。原因は
  **`text-[10px]` が font-size だけを出力し、行送りを指定しない**こと
  ——Tailwind preflight の `html{line-height:1.5}` を継承して1行 15px になる。
  `leading-3`（12px）で明示し、点を `top-5`（20px）から始めて 58px に収めた
  （実測で余裕6px、`scrollHeight == clientHeight`）。**軌道を `h-20` に戻す
  形で直すと上の `17rem` を測り直すことになり**、`a239dc3` が確保した
  「選ぶ場所と押す場所が同時に見える」を削る。`tests/test_day_band_geometry.py`
  が算術を検査する（軌道を縮める・行を足す・行送りの明示を落とす、の3方向で
  落ちる）。
...
**上パディング160pxが字幕ゾーン（当時350px。いまは440px——下の「下端の 150px は
プラットフォームの UI に明け渡す」を参照）の高さを食い**、テキストに残るのは
...
#### 下端の 150px はプラットフォームの UI に明け渡す

**2026-08-23 まで、Shorts の再生画面では字幕の最終行が必ず読めなかった。**
YouTube はフレーム下端の約150pxを自前の UI（チャンネルアイコン・チャンネル名・
タイトル・シークバー）で覆う。実測（オーナーの端末のスクリーンショット、
1080x1920 換算）で UI の上端は y=1768。字幕の文字の下端は `Subtitle.tsx` の
`PADDING_BOTTOM = 72` から y=1848 で、**行数に関わらず最終行が丸ごと UI の
内側**にあった（1行の字幕はその1行が読めない）。`PADDING_BOTTOM` のコメント自身が
「プラットフォームの UI が重なるので 0 にはしない」と書いていたが、**72px は
実測の半分未満**だった。ffprobe では気付けない——尺・解像度・音声トラックは
すべて正しい。

セーフエリアは `zones.ts` の `SAFE_BOTTOM`（形式ごとの比率）に置き、
`Subtitle.tsx` が `max(MIN_PADDING_BOTTOM, safeBottom + BOTTOM_CLEARANCE)` で
下パディングを導く。形式名は props で渡す——**解像度からは決められない**
（short と tiktok は同じ 1080x1920）。

- **`tiktok` は未実測で short から値を借りている。** TikTok の UI は Shorts より
  広く覆うはずなので、測ったらそこだけ差し替える。
- **`long` は 0。** 横画面の通常動画では UI が常時表示ではない。導出値が 12px に
  なるので下限（72px、従来値）が効く。下限を外すと長尺で文字がフレーム下端に寄る。
- **オーナーにだけ出る「このショート動画を宣伝する」ボタン（205px）を基準に
  しない。** 一般の視聴者には出ないので、守るべき最小値は 150px の方。

**Google 公式のセーフエリアは採らない（一次情報だが面が違う）。** 公式の
オーバーレイ画像（`youtubesafezoneoverlay_vertical_final.png`、[About video ad
specs](https://support.google.com/google-ads/answer/13547298) からリンク）を
実測すると、1080x1920 の安全域は x 48..887 / y 288..1247——**下 672px・右 192px・
上 288px**を空けろという値だった。あれは*広告*在庫（CTA バナーと見出しを重ねる）の
セーフゾーンで、下 672px を空けると使えるフレームは 960px しか残らず、挿絵・
見出し・字幕の構成が成立しない。YouTube ヘルプの Shorts 側と TikTok のヘルプには
**数値が載っていない**（TikTok は配布 zip の中だけ）。なお右 192px は Shorts の
右側ボタン列とほぼ一致するので、そちらを測るときの手掛かりとして記録しておく
（字幕は左寄せなので、いま当たりうるのは右端まで届く長い行だけ）。

**レバーは `PADDING_BOTTOM` だが、そこだけ動かすと字幕が縮む。** 150px を確保すると
350px のゾーンに4行が入らず、`fitSubtitleSize` が長い字幕だけを30台前半まで
縮める（シーンごとに字幕の大きさが変わる = #32 が避けたかった状態）。
**下パディングを増やした量だけゾーンを上へ伸ばして相殺する**のが要点で、
テキストに使える高さは **266px のまま**（440 − 12 − 162 = 350 − 12 − 72）。
この等式が崩れると基準サイズ 46px の根拠も崩れる。

**その 90px は挿絵の帯ではなく、章タグの帯とゾーン間の余白から出した。**

| ゾーン | 変更前 | 変更後 |
|---|---|---|
| `shared.illustration` | 0..800 | 変更なし |
| `shared.chapter` | 800..920 | 800..896 |
| `strip.headline` | 970..1370 | 920..1320 |
| `strip.relation` | 1410..1530 | 1340..1460 |
| `strip.subtitle` | 1570..1920 | 1480..1920 |
| `statement.headline` | 1050..1570 | 960..1480 |

- **見出しの高さ（400 / 520）は変えない。** `Headline.tsx` はフォントサイズを
  `zone.height` から逆算するので、削ると 320→400 で直した「45字が欠ける」側へ
  戻る方向に効く。上へ動かすだけなら見た目は変わらない。
- **挿絵の帯を縮めない。** アスペクト比が変わり、`ILLUSTRATION_SIZE` の見直しと
  画像の再検証が連鎖する（クォータはリージョン単位で上限4、動画生成と共食い）。
- **章タグの帯は 76px を下回れない**（`ChapterTag.tsx` のタグの高さが固定 76px）。
  96px は上下に10pxだけ残る値で、**ここが今回いちばん詰まった箇所**。
- **関係ストリップの高さ 120 も変えない。** プレートの上端がゾーン上端の 8px 下に
  収まることを計算で言えるようにしてある（「関係ラベルは…」の節）。

検査は2段。`tests/test_remotion_design_rules.py` が定数だけで算術の不変条件を見る
（slow マーカーが無いので常に走る）。`tests/test_remotion_render_slow.py` が実際に
焼いたフレームで `y=1770..1919` にインクが1画素も無いことを3レイアウトで測る
（受け入れ条件そのもの。目視で判定しない——挿絵の切り取りで目視判断を2回とも
外した前例がある）。**下から持ち上げれば上で切れうる**ので、ゾーン上端の
切れの検査と同時に通らなければ意味がない。

**ゾーンの高さは `zones.ts` から正規表現で読むように変えた。** 以前は
`zone_height = 350` の手写しで、コメントに「`zones.ts` を触ったらここも直す」と
書いてあった——まさにその値を変える今回の修正で写し忘れれば、**検査は古い高さで
計算して通ってしまう**。
...
- **字幕の基準サイズ 46px は「4行が収まる最大値」から決めた値で、読みやすさの
  ...
  参照）。スマホで小さすぎると分かった場合、レバーは**ゾーンを広げること**で、
  自動縮小のロジックを変えることではない。2026-08-23 にそのレバーを実際に引いた
  （350→440）が、**増えた 90px はプラットフォームの UI のセーフエリアに使ったので、
  テキストに使える高さは 266px のまま**である。文字を大きくするには、もう一度
  上のゾーンから借りる必要がある（`statement.headline` の520pxは45字の見出しに
  対して余裕がある）。ゾーンを広げると `ILLUSTRATION_SIZE` や見出しの予算に
  波及するので、まとめて測り直す。
...
さらに**全件が `gpt-image-2` を呼ぶ**（Issue #23 以降。以前は4件に1件）。そのクォータは
**リージョン単位で、動画生成と共食いする**。誰も見ない出力のために動画生成を遅くすることになる。
...
**「媒体名とリンクを両方置く」は取り消された記述。** すぐ上の
「`出典: {媒体名}` は書かない」が現在の判断で、`_assemble` の実装もそちら。
残してあるのは、取り消しの理由が次の勘違いを招くため。

- 当時の理由は「URL だけにすると、画像を添える `CARD` では X がリンクカードを
  描かないためどこの記事か分からなくなる」だった。**Issue #23 で全件が画像を
  持つようになったので、この理由は全件に当てはまることになった。**
- **それでも媒体名は書き戻さない。** X が画像と排他にするのは
  **リンクプレビューのカード（ドメイン＋記事タイトルの箱）だけ**で、
  リンク自体は本文に残りクリックできる（`_assemble` が末尾に足し、
  `x_post_preview.html:39` が画像の有無に関わらずリンクとして描く）。
  X はリンクをドメイン込みの短縮形で表示するので、失うのは
  **記事タイトルの自動表示だけ**。本文（95〜125字・独自解説つき）が
  何のニュースかを既に言っているので、重複に近い。
- 書き戻すと `BUDGETS[SINGLE]` の上限を 125 → 110 に下げることになり
  （`出典: {媒体名}` が最長22カウント）、独自解説の余地が15字減る。
  **払う対価が得るものに見合わない。**
...
### 画像が添付されない経路は2つある（どちらも実物の投稿を見るまで気付けない）

症状は同じ。**画像を作って Blob に上げたうえで添付せずに投稿する。**
生成も保存も成功し、ログにも異常が出ず、`gpt-image-2` のクォータ
（リージョン単位、動画生成と共食い）を消費して誰も見ない画像を作り続ける。

**1. `PostWorker` に `fetch_image` を渡し忘れる。** `post_due_once` の
`media_ids` が常に None になる。2026-08-22 まで実際にそうなっていた
（`AppContext.build` が渡し忘れていた）。`tests/test_scheduler_wiring.py` が
`on_posted`（I4）と同じ形で見張っている。

**2. 添付の条件に投稿の型を入れる。** `post_due_once` の判定は
`if post.image_key and fetch_image is not None:` で、**`kind` を見ない**。
2026-08-23 まで `post.kind is PostKind.CARD` も条件に入っていた。画像を
持てるのが CARD だけだった頃は同義だったが、日次計画が全件を SINGLE +
画像で積むようになった時点（Issue #23）で、この1行が症状そのものになる。
`tests/test_post_worker.py` の `test_画像は型に関係なくメディアを先に上げる`
が SINGLE と CARD の両方で検査しており、型を条件に戻すと SINGLE 側だけが
落ちる（実際に戻して確認した）。

**画像は型ではなく `image_key` が表す。** DB の列（`social_posts.image_key`）も
テンプレート（`post_queue.html` / `x_post_preview.html` の `{% if post.image_key %}`）も
最初から `kind` を見ていない。**送信側だけが型で絞っていた**のがこの欠陥だった。
...
### 画像は投稿の型から独立している（全件に付く）

日次計画は `DAILY_POST_KIND = PostKind.SINGLE` だけを積み、**全件に画像を付ける**。
以前は `KIND_ROTATION`（`SINGLE, SINGLE, SINGLE, CARD`）が4件に1件だけ `CARD` を
割り当て、画像を作るのは CARD のときだけだった。2026-08-17 の3件は投稿枠が3つしか
残らなかったため割り当てが `SINGLE, SINGLE, SINGLE` になり、**画像が1枚も付かなかった**
（Issue #23）。

**画像を型に結び付けていたことが欠陥だった。** DB の列もテンプレートも最初から
`image_key` だけを見ていて、型で絞っていたのは計画側と送信側の2行だけ。
そこを外したので「CARD から SINGLE への降格」という概念も消えた
（本文の予算は常に 95〜125字なので、画像の失敗で型を切り替える理由が無い）。

- **`PostKind.CARD` は消さずに未配線で残す。** `src/storage/social.py` の
  `_to_domain` が `PostKind(record.kind)` で復元するので、`kind='card'` の既存行が
  あると `ValueError`。その経路は `monthly_post_counts`（予算判定）の上にあり、
  計画のたびと `PostWorker` の毎周で通るため、表示バグではなく**クラッシュ**になる。
  テスト DB は毎回作り直すので既存のテストでは捕まらない。短い本文（60〜90字・
  画像主役）に戻す退路としても意味がある。
- **クォータは律速ではない。** Issue #23 は #10（クォータ引き上げ）待ちで保留されて
  いたが、実測して前提が変わった。カードの画像生成は**1記事1リクエストの逐次**
  （`generate_batch([prompt], ...)` に対し `workers = min(max_concurrency, 1) = 1`）で、
  間に `CardVisualGenerator` の LLM 呼び出しが挟まる。1枚17〜37秒なので60秒窓に
  入るのは2〜3件で、westus3 の capacity 4（= 4リクエスト/分）に当たらない。
  **並行に投げる形に変えると当たる。**
  実測（2026-08-23）: サブスクリプション上限は 4 → **6** に上がっていたが、
  デプロイの capacity は **4 のまま**（#10 の反映は未実施）。
- **`has_active_jobs()` の射程が広がった。** この判定に引っかかると、以前は
  画像1枚を失うだけだったが、いまは**その日の全件**が画像なしになる。通常は
  起きない——日次タスクは「取得 → X の計画 → 動画の投入」の順なので、X の
  計画時にはまだジョブが無い（`src/web/dependencies.py` の `task()` の理由1）。
  起きるのは前サイクルの詰まったジョブが残っているか、手動生成とぶつかった
  ときだけ。
- **引き換えに動画バッチの投入が3〜6分ずれる**（カード4件の生成時間）。動画側の
  タイムアウトは900秒・リースは15分で、投入前の遅延なので影響しない。
- **画像を作った後に本文が拒否されると、その画像は捨てられる。** 順序を入れ替えると
  キャプションのヒント（画像に描かれた文を本文で繰り返さない）を本文生成に渡せない。
  拒否された記事は `mark_content_filtered` で二度と選ばれないので、無駄は
  **記事1件につき1枚・1回だけ**に収まる。順序は変えない。
- **`gpt-image-2` の1枚あたりの単価は未計測のまま**（1日1枚 → 4枚になった）。
  X 側の課金は変わらない（メディア添付は投稿単価を変えず、`has_link` は既に常に
  True）。増えるのは Azure 側で、`src/social/cost.py` の費用モデルに画像生成の項は
  無い。Retail Prices API は `serviceName eq 'Cognitive Services'` では0件を返す。
