...
**DB が共有になってこの注意の重みが増した**（2026-08-23）。以前はレプリカごとに
別の SQLite ファイルだったので、同時に走っても互いに干渉しなかった。いまは
同じ PostgreSQL に対して同時に DDL を流すことになる。レプリカが1で、
デプロイ中に同時に起動するのは新リビジョンだけ（旧は upgrade を走らせない）
なので現状は当たらないが、**レプリカを増やす前にここを切り出す**。
"""
