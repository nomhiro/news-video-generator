...
リビジョン更新で消えた。指標は蓄積してこそ意味を持つデータなので、そこには
置けなかった（DB は 2026-08-23 に共有の PostgreSQL へ移して消えなくなったが、
時系列の測定値を投稿表に足す必要は無いのでここは変えていない）。
...
def collect_metrics(
    ...
    # その `posted_at` は投稿表にしか無い（当時はデプロイごとに消える
    # SQLite だったので、Blob に書いていた理由が1回のマージで無くなった）。
    # 行が残るようになった今でも、**ファイル1枚を単体で解釈できる**ことに
    # 価値があるので offset は行に持たせ続ける。
    ...
    if not targets:
        # **ファイルは書かないが、走ったことはログに残す。**
        # 空のファイルを毎日置くと Blob にごみが積もるので書かない。ただし
        # 何も書かず何も言わないと、運用者から見て「走って対象が0件だった」と
        # 「そもそも走っていない」が区別できない。無人で回すものは、
        # 動いている証跡が無いのが一番困る（実際に初日の 11:00 に走ったか
        # どうかがログから判断できなかった）。
        log_step("計測: 対象の投稿がありませんでした（24時間後・7日後の窓に該当なし）", "📈")
        return 0
