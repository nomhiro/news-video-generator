...
def _reach_context(
    ...
    **記事データだけでは足りない。** 公開の記録は動画ファイル単位の別ストア
    （`data/publications.json`）にあり、X の下書きは投稿表（DB）にある。
    記事に持たせない理由は前者が粒度（1記事から複数の動画）、後者が
    「下書きは日々入れ替わる状態で、記事の属性ではない」こと。
...
async def x_set_enabled(
    ...
    実体は Azure Files 上のファイル。消える置き場所に置くと、画面で
    有効にした翌日にマージした時点で黙って止まる（`src/social/switch.py`）。
    """
