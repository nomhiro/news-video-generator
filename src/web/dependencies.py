    ...
    **DailyScheduler は1つだけ。** 動画の計画と X 投稿の計画を
    ...
    Args:
        ...
        jobs: ジョブ表。`plan_daily_posts` に渡し、動画生成が実行中の間は
            画像を作らせない（`gpt-image-2` のクォータをリージョン単位で
            動画パイプラインと共有しているため）。**全件が画像を持つので、
            ここに引っかかるとその日の全件が画像なしになる**（Issue #23）
        posts: 投稿表
        ...
        card_generator: 投稿に添える画像の視覚指示生成器
        image_generator: その画像の生成器。`Pipeline` が持つものを
    ...
    async def task() -> object:
        ...
        # ジョブをジョブ表に積む。`plan_daily_posts` は画像を作る前に
        # `jobs.has_active_jobs()` を見て、実行待ち・実行中のジョブが1件でも
        # あれば画像を諦める（`gpt-image-2` のクォータはリージョン単位で、
        # 動画パイプラインと共有しているため）。動画を先に積むとこの判定が
        # **常に True** になり、画像の機能全体（視覚指示・画像生成・Blob への
        # 保存・メディア添付）が本番で一度も動かない。
        #
        # **Issue #23 で射程が広がった。** 画像が4件に1件だった頃、順序を
        # 間違えて失うのは1枚だけだったが、いまは**その日の全件**が画像なし
        # になる。引き換えに X が先に使うクォータも増えた（1枚 → 4枚。
        # remotion 既定では動画1本の挿絵は1枚）。それでも X を先にするのが
        # 正しい: カードの画像生成は1記事1リクエストの逐次で、`gpt-image-2`
        # の毎分クォータに当たらないまま数分かけて流れるだけなので、動画側は
        # 投入が遅れるだけで待ち時間は増えない。
        #
        ...
        # `test_動画ジョブを積んでも画像は作られる` が両方を見張っている。
